...
b\) To examine whether daily workload (hours per day) predicts whether or not I went in the ocean (binomial, yes ocean/no ocean), I would use logistic regression. A generalized linear model (GLM) is appropriate because the response variable is binomial, and the predictor variable (hours worked per day) is continuous. This analysis allows us to model how changes in workload influence the probability of participating in an ocean activity.

c\)

```{r}
personal_clean <- my_data |> # creating a new object called personal_clean
  clean_names() |> # cleaning the names
  mutate(ocean_bin = ifelse(ocean == "Yes", 1, 0)) |> # making a new column that displays 1 if ocean is in yes and 0 if not
  select(ocean_bin, workload_hours) #selecting only binary response and predictor
```

```{r}
personal_mod <- glm(ocean_bin ~ workload_hours, # creating a glm, ocean_bin as a function of workload
                    data = personal_clean, # data
                    family = "binomial") 
```

```{r}
plot(
  simulateResiduals(personal_mod) # using DHARMa to plot residuals
)
```

```{r}
summary(personal_mod) # displaying analysis
```

```{r}
gtsummary::tbl_regression(personal_mod,
                          exponentiate = TRUE) # displaying analysis in a table
```

```{r}
exp(coef(personal_mod)) # displaying effect size
```

Logistic regression assumes independence of observations, binomial distribution, a linear relationship between transformed expected response in terms of link function and explanatory variables. Observations were assumed independent. However the DHARMa package indicated quantile deviations. This means that the model may not fully capture the relationship between workload and ocean use.

d\)

```{r}
personal_pred <- ggpredict(personal_mod,
                       terms = "workload_hours") # predicted probabilites

ggplot(personal_clean, # using mopl_clean data frame
       aes(x = workload_hours, # workload_hours as x-axis
           y = ocean_bin)) + # ocean participation as y-axis
  geom_point(aes(color = "Observed"), # coloring data points
             size = 3, # setting the size and transparency 
             alpha = 0.4)  + 
  geom_ribbon(data = personal_pred, # using personal_pred for ribbon data
              aes(x = x, # keeping the same x-axis
                  y = predicted, 
                  ymin = conf.low,
                  ymax = conf.high),
              fill = "lightblue", # coloring it light blue and setting transparaency
              alpha = 0.4) + 
  geom_line(data = personal_pred, # using personal_pred data for line
            aes(x = x, # keeping the same x-axis
                y = predicted,
                color = "Model"), # coloring the line and setting width
            linewidth = 1) +
  labs(x = "Daily Workload (hours)",  # labeling the x and y axis
  y = "Probability of Ocean Activity Participation") +
  scale_y_continuous(limits = c(0, 1), # setting y-axis scale
                     breaks = c(0, 1)) + 
  scale_color_manual(values = c("Observed" = "grey67", # manually coiloring the line and data points
                                "Model" = "blue")) +
...
e\) Figure d: Probability of ocean activity participation decreases as daily workload (hours) increases. The predicted probability of me going in the ocean (y-axis) is shown as the response of my daily workload (x-axis), based on a logistic regression model. The dark blue line shows the predicted probability of ocean activity participation and the shaded area around it represents the 95% confidence interval. The grey, transparent points at the top and bottom of the graph represent the observed data of ocean activity participation, with a one corresponding to getting in the ocean and a zero corresponding to not getting in the ocean.

f\) I was significantly less likely to enter the ocean as daily workload increased, because as I spent more time on homework or in class, I was less likely to have time to go in the ocean. With every one hour increase in workload, the odds of entering the ocean decreased by a factor of 0.65 (95% CI: \[0.48, 0.83\], p = 0.0017, ⍺ = 0.05). At higher workload levels, the predicted probability of entering the ocean was low, whereas at lower workload levels, the probability was substantially higher. The effect size, which is the same as the odds ratio (0.65) allows us to quantify the magnitude of the relationship, or how strongly daily workload influences ocean participation.

g\) Shared my visualization in class!
